...
cerrar.addEventListener('click' , () => { //click para botones q estan fuera del formulario
    if (productos>0){
...
fab.addEventListener('click' , () => { //click para botones q estan fuera del formulario
...
volver.addEventListener('click' , () => { //vuelve desde el detalle al listado 
    formulario.classList.add('dnone')
    listado.classList.remove('dnone')
    home.classList.add('dnone')
    detalle.classList.add('dnone')
...
const addItem = (titulo, categoria, descripcion) => { //se crea una variable q como valor tiene un fragmento de html 
...
form.addEventListener('submit', (e) => { //submit envia los datos de un formulario ((aplica al ultimo boton dentro de un formulario))
    e.preventDefault()
